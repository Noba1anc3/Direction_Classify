num = {'chepiao': [0, 0, 0, 0, 0, 0, 0, 0],
       ...
       'gongsibaobiao': [0, 0, 0, 0, 0, 0, 0, 0],
       'shenfengzheng': [0, 0, 0, 0, 0, 0, 0, 0],
...
Certificate = {'mingpian', 'shenfengzheng', 'jiashizheng', 'yingyezhizhao'}
certificate = [0, 0, 0, 0, 0, 0, 0, 0]
Docu = {'dianzishu', 'jiaocai', 'muji', 'yinhangliushui', 'pingjibaogao', 'gongsibaobiao'}
...
with open('./label_1.txt', 'r') as f:
    lines = f.readlines()
    for line in lines:
           labels.append([line.split(' ')[0], line.split(' ')[2].split('\n')[0]])
f.close()

with open('./result.txt', 'r') as f:
    lines = f.readlines()
    for line in lines:
           preds.append([line.split(' ')[0], line.split(' ')[3].split('\n')[0], line.split(' ')[1], line.split(' ')[2]])
f.close()
...
print('检测时间最少:', phase_1_min, '检测时间最多:', phase_1_max, '检测时间平均:', '%.2f' % (phase_1_time / 3600))
